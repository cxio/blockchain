...
- **`BigInt` 二进制序列化**：脚本类型 `BigInt` 在哈希和签名输入中按下列字节布局序列化：
    ...
    - `slen` 为 1 字节附参：最高位（bit7）为符号位（`0` 正数，`1` 负数），低 7 位为 `magnitude` 字节长度（`0-127`）。
    - `magnitude` 为绝对值字节序列，使用大端无符号表示。
    - `magnitude` 的实际长度必须与 `slen` 低 7 位严格一致；超过 `127` 字节非法。
    - 零值必须编码为 `slen=0x00` 且 `magnitude` 为空；非零值的 `magnitude` 首字节不得为 `0x00`（最短大端编码）。
    - 当长度为 `0` 时，符号位必须为 `0`（禁止负零编码）。

## Rationale（理由）

大端固定宽度编码便于人工审查和跨语言实现。ULEB128 简单、成熟，且能高效表达输出序位、年度、附件大小等小整数。最短编码规则可避免同一数据拥有多个 TxID 或 BlockID。

默认 varint、定宽字段白名单和"年度采用 UTC 自然年份"源自 conception 修订。`BigInt` 采用与值指令定义一致的“符号位+长度附参”布局，可避免同一类型在脚本字节码与哈希/签名输入中出现双重编码规则，降低跨语言实现歧义。
...
- `BigInt` 跨语言实现必须提供针对零值、正数、负数、单字节绝对值、最大长度（127 字节）、长度不一致、负零等边界的字节级测试向量。
