...
1. `Config byte`：高 4 位为位标记，低 4 位为类型（`1`=币金 / `2`=凭信 / `3`=存证 / `0` 预留 / `bit7=1` 自定义类）。**不包含销毁位**——普通交易不可销毁币金，销毁仅由 Coinbase 的 `BurnCoin` 字段表达（详见 DEC-0401）。
2. `Payload bytes`
3. `LockScript bytes`
...
### 自定义类输出（Config bit7=1）

- 自定义类输出可出现在交易中，但**不进入 UTXO/UTCO 集合**，因此不能作为后续交易的输入源，与 conception `附.交易.md` 一致。
- 自定义类输出的 `Payload` 与 `LockScript` 字段仍按公共头规则编码，便于通用解析。
- 节点对自定义类输出仅做编码合法性校验，不做语义校验。
...
自定义类输出不进入 UTXO/UTCO，与 conception `附.交易.md` 中"自定义类不能作为输入源"一致，保留链上扩展空间但不增加状态压力。
...
- 自定义类输出虽不进入 UTXO/UTCO，但其编码合法性仍由共识规则校验；节点必须能完整解析以参与区块哈希计算。
