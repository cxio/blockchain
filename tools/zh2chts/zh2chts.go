...
func main() {
	if len(os.Args) < 2 || os.Args[1] == "-h" {
		fmt.Fprintln(os.Stderr, usage[1:])
		os.Exit(0)
	}
	dir, nameFilter, method := os.Args[1], ".*", "cn2tw"
	if len(os.Args) > 2 {
...
func convertFile(path, method string) (changed bool) {
	...
	switch {
	case method == "cn2tw":
		newData = []byte(cn2tw(string(oldData)))
	case method == "tw2cn":
		newData = []byte(tw2cn(string(oldData)))
	}
...
func cn2tw(s string) string {
	old := []rune(s)
	new := make([]rune, 0)
	for _, c := range old {
		if x, ok := cn2twMap[c]; ok {
			new = append(new, x)
		} else {
			new = append(new, c)
		}
...
func tw2cn(s string) string {
	old := []rune(s)
	new := make([]rune, 0)
	for _, c := range old {
		if x, ok := tw2cnMap[c]; ok {
			new = append(new, x)
		} else {
			new = append(new, c)
		}
...
// 简体转繁体。
// 一个简体可能对应多个繁体，这里仅采用key键序中的首个繁体。
// 注：越常见的字其Unicode值越低。
var cn2twMap = make(map[rune]rune)

func init() {
	// key键序排列。
	kkMap := make([]int, 0, len(tw2cnMap))
	for k := range tw2cnMap {
	...
	for _, k := range kkMap {
		...
		// 保留首个繁体。
		if _, ok := cn2twMap[v]; !ok {
			cn2twMap[v] = k
		}
	}

	// 修正不当转换（仅简转繁）
	for k, v := range cn2twMapPatch {
		cn2twMap[k] = v
	}
}

// 简-繁转换不当纠正。
var cn2twMapPatch = map[rune]rune{
	'台': '臺',
	'当': '當',
	'录': '錄',
	'证': '證',
