...
本设计试图克服以上这些缺点，创建出一个**能源节约、安全性足够、同时也有良好去中心化特性**的区块链系统，以及各系统之间协作的机制和环境。
...
这里，用户发起交易，交易携带意图，通过区块链传递到网络的任何一个局部子系统里。或许，这可以称之为**交易驱动**的模型。
...
交易定义了权属在个体之间的转移，获取信息、构建交易并发送，在逻辑上是个体私有的。因此，从一条链上获取信息创建另一条链上的交易并不涉及公共验证，也即，链间的信用传递实际上需要自负其责。私有的环境应当安全，但这与公共域无关。
...
- 用户将A链代币兑换为B链代币，注册中间件监听对方行为实现自动化处理（见：[链间兑换](examples/链间兑换.md) 脚本示例）。
...
1. **一级筛选**：交易ID与动态的目标区块（`-11` 号）的哈希相似性评选。
...
在这个星球上，除了大陆和海洋，还有流动的空气、热力学的定律，市场的驱动力……世界是全息的。区块链的信用能力更应当是一种力量、动能。因为信用和安全，它或许拥有驱动这个社会运行的能力，甚至改善人类未来与机器的关系。

最后引用拜占庭将军的问题：将军们现在终于有了一个可靠的信令系统，他们可以协调一致了。
