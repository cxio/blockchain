...
交易ID拥有随机性，我们可以据此设计出一个基于概率的共识模型。它不需要工作量（PoW），因此可能非常廉价。与传统的权益证明（PoS）不同，交易ID只是一个哈希值，与财富无关，因此富者越富的情况可能会缓解很多。
...
*&gt;> 用逻辑设计程序，用代码书写逻辑，用程序表达自然。And algorithm, Through along*
