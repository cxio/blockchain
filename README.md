...
4. **缺乏时间确定性：**
靠碰运气的哈希竞争缺乏时间确定性，出块时间无法确定。这可能不是一个大问题，但如果时间能够固定会更好。

5. **庞大的数据负担：**
区块数据持续增长，存储和计算的成本也越来越高。这在P2P逻辑下不是一件好事，自由离散节点的成本会越来越大。

本设计彻底重新思考，在遵循区块链核心原理及其优势的前提下，克服了以上全部不足。同时还增强了安全性、通用性，并提供了灵活的栈脚本。
...
// 解锁脚本
// 注：所有的返回值都会自动入栈。
DATA[32]{<hashSource>}              // 哈希源，由甲方创建并保留
{2}                                 // 验证类型（多签）
<[sig]>                             // 签名数据
<[pubKey]>                          // 公钥序列
<[pubHash]>                         // 公钥哈希（地址）序列
...
> **详见**：examples/[链间兑换.md](examples/链间兑换.md)。

完整指令说明请见：[脚本基础指令集（明细）](6.脚本基础指令集.md)。
