...
本设计试图克服以上这些缺点，创建出一个**能源节约、安全性足够、同时也有良好去中心化特性**的区块链系统，以及各系统之间相互协作的机制和环境。
...
2. **二级筛选**：交易输入项的币权（`币量x币龄`）大小筛选。
3. **三级筛选**：保证最终唯一性。取一级筛选中某个哈希值简单比较即可。

这样的设计有如下优点：

1. 与传统的PoS权益证明不同，交易ID只是一个数据哈希，与财富无关。币权作为次级因子只是利用币权自身的价值，不会导致富者越富。
2. 用交易ID参与铸造竞争会鼓励交易，大量的交易费能够保证矿工的收益，这是一种良性回馈。
...
在这个星球上，除了大陆和海洋，还有流动的空气、热力学的定律，市场的驱动力……世界是全息的。区块链的信用能力更应当是一种力量、动能，它或许拥有驱动这个社会运行的能力，甚至改善人类未来与机器的关系。
